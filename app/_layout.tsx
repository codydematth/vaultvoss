...
import { ActivityIndicator, AppState, View, Image, StyleSheet } from "react-native";
...
function InitialLayout() {
    ...
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      ...
      if (nextAppState === "active") {
        if (backgroundTimeRef.current !== null) {
          const elapsedMin =
            (Date.now() - backgroundTimeRef.current) / (1000 * 60);

          if (elapsedMin >= 5 && isAuthenticated) {
            clearSession().catch((err) =>
              console.log("Timeout logout failed:", err),
            );
          }
  ...
  useEffect(() => {
    if (isAuthenticated) {
      import("@/lib/notifications")
        .then((module) => {
          if (typeof module.registerPushNotifications === "function") {
            module
  ...
  const showSecureOverlay = isAuthenticated && currentAppState.match(/inactive|background/);
