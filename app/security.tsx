...
export default function SecurityScreen() {
  ...
  const handleToggle = async (value: boolean) => {
    ...
    if (!hasHardware || !isEnrolled) {
      setError('Biometric hardware is not configured or enrolled on this device.');
  ...
  const handleConfirmPassword = async () => {
    ...
    try {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      showToast({message: `${biometricLabel} login enabled successfully`, type: 'success'});
    } catch (err: any) {
  ...
  return (
    <View style={{flex: 1, backgroundColor: C.bg}}>
      {/* Header */}
      <View style={{paddingTop: (insets.top ?? 0) + 16, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 20}}>
        <Pressable
        ...
        <Text variant="heading" color="primary">Security</Text>
      </View>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingHorizontal: 20, gap: 20}}>
        ...
        <View
          ...
          <View style={{flex: 1, gap: 2}}>
            <Text variant="subheading" style={{fontFamily: Fonts.sansBold, fontSize: 15, color: hasHardware ? C.income : C.expense}}>
              {hasHardware ? 'Biometrics Supported' : 'Hardware Not Supported'}
            </Text>
            <Text variant="caption" color="secondary" style={{fontSize: 12, lineHeight: 16}}>
              {hasHardware
        ...
        <View
          ...
          <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
            <View style={{flex: 1, gap: 2, paddingRight: 12}}>
              <Text variant="subheading" color="primary">{biometricLabel} Login</Text>
              <Text variant="caption" color="secondary" style={{fontSize: 12, lineHeight: 16}}>
                Fast, secure sign-in to your financial dashboard using your biometric scanner.
              </Text>
            </View>
            <Switch
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View
            ...
            <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20}}>
              <Text variant="title" color="primary">Confirm Password</Text>
            ...
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <View style={{gap: 16, paddingBottom: 20}}>
                <Text variant="body" color="secondary" style={{lineHeight: 22}}>
                  To enable biometric login, please enter your current account password to verify your credentials.
                </Text>
